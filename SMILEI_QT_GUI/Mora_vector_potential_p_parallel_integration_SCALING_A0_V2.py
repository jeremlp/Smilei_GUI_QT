...
for sim in sim_loc_list:
    ...
    Int_AzM2_vx = integrate.simpson(AzM**2*1/(1-vx[:,Nid]),x=t_range[:])
    Int_AzM2_gammaP = integrate.simpson(AzM**2*gamma[:,Nid],x=t_range[:])
    Int_AzM2_gammaA0 = integrate.simpson(AzM**2*sqrt(1+a0**2),x=t_range[:])
    Int_AzM2_gammaA02 = integrate.simpson(AzM**2*sqrt(1+a0**2/2),x=t_range[:])
    
    Int_AxM02 = integrate.simpson(AzM0**2*sqrt(1+a0**2/2),x=t_range[:])
...
fig.suptitle('Integrated "Intensity" $\int A.B~dt$')
...
ax.legend()
fig.suptitle('Integrated "Intensity" $\int A.B~dt$ normalized')
ax.set_xlim(0.9,3.1)
# ax.set_xscale("log")
# ax.set_yscale("log")
fig.tight_layout()
