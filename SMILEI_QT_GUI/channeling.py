...
S2 = happi.Open('C:/_DOSSIERS_PC/_STAGE_LULI_/CLUSTER/SIM_PHYSICAL/sim_base_SAM_Long')
...
plt.imshow(weight2.T, cmap="jet",aspect="auto",vmin=1,vmax=1.5, extent=[x_range[0], x_range[-1], r_range[0],r_range[-1]])
plt.colorbar()
plt.tight_layout()
...
for x_idx in range(len(x_range)):
    ...
    peaks2, _ = find_peaks(weight2[x_idx], height=1.1, prominence=0.35)
    ...
    if len(peaks) >=2:
        ...
        if l_channel >1.5:
            ...
            plt.scatter(x_range[x_idx],r_range[peaks[0]],marker="x",color="green")
            plt.scatter(x_range[x_idx],r_range[peaks[-1]],marker="x",color="green")
