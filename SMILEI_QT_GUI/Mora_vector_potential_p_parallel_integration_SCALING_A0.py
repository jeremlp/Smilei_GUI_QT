...
for sim in sim_loc_list:
    ...
    def f_nuter(r,z):
        return C_lp * (r/w(z))**abs(l1)*exp(-1.0*(r/w(z))**2)
    ...
    def Rc(z):
        return z*(1+(zR/z)**2)
    def phi(z,r,t):
        return z - t + ( r**2/(2*Rc(z)) ) - (abs(l1)+1)*arctan2(z,zR)
    ...
    Int_AxPx = integrate.simpson(AzM*px[:,Nid],x=t_range[:])
...
ax.plot(a0_range,Int_Ex2_LIST/Int_Ex2_LIST[0],"k.-",label="Ex^2",lw=2)
ax.plot(a0_range,Int_ExM2_LIST/Int_ExM2_LIST[0],"k--",label="ExM^2",lw=2)
ax.plot(a0_range,Int_ExM2_LIST/Int_AxM2_LIST[0],"k-.",label="AxM^2",lw=2)

ax.plot(a0_range,Int_AxPx_LIST/Int_AxPx_LIST[0],".-",label="Ax * px",lw=2)
ax.plot(a0_range,Int_AxPx_over_gammaLIST/Int_AxPx_over_gammaLIST[0],"--",label="1/gamma*Ax * px",lw=2)
ax.plot(a0_range,Int_ExM2_LIST/Int_AxM2_LIST[0]*sqrt(1+a0_range**2/2),"r-.",label="AxM^2*$\sqrt{1+a0^2/2}$",lw=2)


# ax.plot(a0_range,Int_AzM2_vx_LIST/Int_AzM2_vx_LIST[0],".-", label="Ax^2 *1/(1-vx)")
# ax.plot(a0_range,1.25*Int_AzM2_vx_LIST/Int_AzM2_vx_LIST[0],".-", label="Ax^2 *1.25/(1-vx)")
...
# ax.plot(a0_range,Int_Ex2_LIST/Int_Ex2_LIST[0]*sqrt(1+a0_range**2),".-",label="Ex^2 *sqrt(1+a0^2)")
...
ax.grid()
ax.set_xlabel("a0")
ax.plot(a0_range,Lx_amplitude_LIST/Lx_amplitude_LIST[0],"x",color="blue",label="Smilei Lx",lw=2)
ax.plot(a0_range_smooth,Lx2_max_model_LIST/Lx2_max_model_LIST[0],"--",color="blue",label="Lx2 model",lw=2)
...
# plt.plot(t_range, gamma[:,Nid]*0+sqrt(1+(a0*g_gauss(t_range-x_pos))**2),"r--",label="$\sqrt{1+a0^2}$")
# plt.plot(t_range, gamma[:,Nid]*0+sqrt(1+(a0*g_gauss(t_range-x_pos))**2/2),"g--",label="$\sqrt{1+a0^2/2}$")
# plt.plot(t_range, gamma[:,Nid]*0+sqrt(1+(exp(-0.5)*(a0*g_gauss(t_range-x_pos)))**2/2),"b--",label="$\sqrt{1+(0.6*a0)^2/2}$")
# plt.legend()
# plt.grid()
