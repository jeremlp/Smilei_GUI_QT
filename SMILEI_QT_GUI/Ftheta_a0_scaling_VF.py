...
Ftheta_Ax2 = 8/3 * -1/4*(-sin(THETA)*Ax2_dY + cos(THETA)*Ax2_dZ)
Ftheta_AxPx = 8/3 * -1/4*(-sin(THETA)*AxPx_dY + cos(THETA)*AxPx_dZ)

# gamma42 = sqrt(1+(f(r0,x0)*a0)**2+ 1/4*(f(r0,x0)*a0)**4)

plt.figure()
plt.scatter(R/l0,Tp*R*Ftheta_Ax2,s=2,label="Lx from <Ax^2>")
plt.scatter(R/l0,sqrt(1+(f(R,5*l0)*a0)**2+ 1/4*(f(R,5*l0)*a0)**4)*Tp*R*Ftheta_AxPx,s=2,label="Lx from <Ax*px>")
plt.grid()
plt.legend()
...
plt.plot(t_range/l0,1/gamma[:,Nid]*r[:,Nid]*Ftheta(r[:,Nid], theta[:,Nid], x[:,Nid])*gauss(t_range,x[:,Nid])**2,".-",label="r*Ftheta exact")
# plt.plot(t_range/l0,(r0) * Ftheta(r0, theta0, x_pos)*gauss(t_range,x[:,Nid])**2,label="r0*Ftheta0 NR")
plt.plot(t_range/l0,1/sqrt(1+(f(r0,x0)*a0)**2/2+ 1/16*(f(r0,x0)*a0)**4) * np.abs(r0+dr(r0,t_range,x_pos)) * Ftheta(np.abs(r0+dr(r0,t_range,x_pos)), theta0, x0)*gauss(t_range,x[:,Nid])**2,label="(r+dr)*Ftheta dr")
plt.grid()
plt.legend()

gamma42 = sqrt(1+(f(r0,x0)*a0)**2/2+ 1/16*(f(r0,x0)*a0)**4)
a = integrate.simpson(1/gamma42 *r[:,Nid]*Ftheta(r[:,Nid], theta[:,Nid], x[:,Nid])*gauss(t_range,x[:,Nid])**2,x=t_range)
b = integrate.simpson(1/gamma42 * np.abs(r0+dr(r0,t_range,x_pos)) * Ftheta(np.abs(r0+dr(r0,t_range,x_pos)), theta0, x0)*gauss(t_range,x[:,Nid])**2, x=t_range)
print("Ftheta integration (exact, r+dr):",a,b)
...
def Lx4_distrib():
    ...
    for r in r_range:
        ...
        for theta in np.arange(0,2*pi,pi/16):
            LxR_distrib = integrate.simpson(np.abs(r+dr(r,t_range_lx,x_pos)) * Ftheta(np.abs(r+dr(r,t_range_lx,x_pos)), theta, x_pos)*temp_env**2, x=t_range_lx)
...
def Lx4_distrib_Relat():
    ...
    for r in r_range:
        ...
        for theta in np.arange(0,2*pi,pi/16):
            # if x[:,Nid] is np.nan:
            #     Lx_theta_list.append(0)
            #     continue
            LxR_distrib = integrate.simpson( np.abs(r+dr_Relat(r,t_range_lx,x_pos)) * Ftheta(np.abs(r+dr_Relat(r,t_range_lx,x_pos)), theta, x_pos)*temp_env**2, x=t_range_lx)
            Lx_theta_list.append(LxR_distrib)
        distrib_Lx_list.append(np.max(Lx_theta_list))
    return np.array(r_range),np.array(distrib_Lx_list)

def Lx4_distrib_Relat_Mean():
    t_range_lx = t_range#np.arange(0,t_range[-1],dt)
    temp_env = gauss(t_range,x_pos)
    distrib_Lx_list = []
    for r in r_range:
        ...
        for theta in np.arange(0,2*pi,pi/16):
            ...
            #     Lx_theta_list.append(0)
            #     continue
            LxR_distrib = integrate.simpson( np.abs(r+dr_Relat_mean(r,t_range_lx,x_pos)) * Ftheta(np.abs(r+dr_Relat_mean(r,t_range_lx,x_pos)), theta, x_pos)*temp_env**2, x=t_range_lx)
            Lx_theta_list.append(LxR_distrib)
        distrib_Lx_list.append(np.max(Lx_theta_list))
    return np.array(r_range),np.array(distrib_Lx_list)
...
def Lx_distrib_FullMotion_HIGH_ACCURACY():
    t_range_lx = t_range#np.arange(0,t_range[-1],dt)
    distrib_r_list = []
    distrib_Lx_list = []
    for Nid in range(len(x[0])):
        temp_env = gauss(t_range,x[:,Nid])
        gamma_px = sqrt(1+(f(r[:,Nid],x[:,Nid])*a0)**2 + 1/4*(f(r[:,Nid],x[:,Nid])*a0)**4)
        LxR_distrib = integrate.simpson(gamma_px/gamma42 * r[:,Nid] * Ftheta(r[:,Nid], theta[:,Nid], x[:,Nid])*temp_env**2, x=t_range_lx)
        distrib_Lx_list.append(LxR_distrib)
        distrib_r_list.append(r[0,Nid])
    return np.array(distrib_r_list),np.array(distrib_Lx_list)
...
plt.plot(r_range/l0,-COEF*Lx_max_model,"k--", label="Model $\gamma$$L_z^{(2)}$")
...
# plt.plot(d_r_list/l0,-COEF*d_Lx_list,"C2--",lw=2, label="Model $\gamma$$L_z^{(4)}$ Relat 1/$\sqrt{1+(f(r)*a_0)^2 + 1/4(f(r)*a0)^4}$")

d_r_list, d_Lx_list = Lx4_distrib_Relat_Mean()
COEF = sqrt(1+(a0*f(d_r_list,x_pos))**2+ 1/4*(a0*f(d_r_list,x_pos))**4)
plt.plot(d_r_list/l0,COEF*d_Lx_list,"C2--",lw=2)
plt.plot(d_r_list/l0,-COEF*d_Lx_list,"C2--",lw=2, label="Model $\gamma$$L_z^{(4)}$ Relat 1/$\sqrt{1+(f(r)*a_0)^2/2 + 1/6(f(r)*a0)^4}$")
...
plt.plot(a_range/l0,upper_Lx,"r-.",lw=2, label="Exact integration (gamma(r+dr))")

d_r_list, d_Lx_list = Lx_distrib_FullMotion_HIGH_ACCURACY()
a_range, lower_Lx, upper_Lx = min_max(d_r_list,d_Lx_list)
plt.plot(a_range/l0,lower_Lx,"-",color="purple",lw=2)
plt.plot(a_range/l0,upper_Lx,"-",color="purple",lw=2, label="Exact integration (gamma(r_exact))")
...
THETA = np.arctan2(Z,Y)
R = np.sqrt(Y**2+Z**2)
plt.scatter(R/l0,2.5*Tp*R*Ftheta_Ax2,s=5,label="Lx from <Ax^2>")
plt.scatter(R/l0,0.5*sqrt(1+(f(R,5*l0)*a0)**2+ 1/4*(f(R,5*l0)*a0)**4)*Tp*R*Ftheta_AxPx,s=5,label="Lx from <Ax*px>")
