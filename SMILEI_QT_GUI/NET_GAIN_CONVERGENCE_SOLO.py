# -*- coding: utf-8 -*-
"""
Created on Tue Feb  4 16:46:35 2025
...
sim_loc_list_12 = ["gauss_a4_Tp12_NET_GAIN_dx64_AM4"]
dx_range_12 = ["dx=$\lambda/64$, AM=4"]
...
for sim, dx_label in zip(sim_loc_list,dx_range):
    ...
    if "+5" in dx_label:
        ls = "--"
    if "a0=4" in dx_label or "+5" in dx_label:
        a_range = a_range[:-4]
        mean_Lx = mean_Lx[:-4]
        std_Lx = std_Lx[:-4]
    ...
    ax2.fill_between(a_range/l0, (mean_Lx-std_Lx*2), (mean_Lx+std_Lx*2),alpha=0.25,color=f"C{k}")
